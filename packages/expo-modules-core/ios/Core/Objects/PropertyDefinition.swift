...
public struct PropertyDefinition<OwnerType>: AnyDefinition, AnyPropertyDefinition, Sendable {
  ...
  /**
   Initializes a property definition with a getter.
   */
  init<ReturnType>(name: String, get: @Sendable @escaping (_ this: OwnerType) -> ReturnType) {
    ...
    getter.takesOwner = OwnerType.self != Never.self
    self.init(name: name, getter: getter, setter: nil)
  }
  ...
  init<ReturnType>(
    ...
    set: (@Sendable (_ this: OwnerType, _ newValue: ReturnType) -> Void)?
  ) {
    ...
    if let set {
      ...
      setter?.takesOwner = OwnerType.self != Never.self
    }
