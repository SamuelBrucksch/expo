...
class FileDownloader(
  ...
  private fun preparePatchBaseAsset(
    ...
    val currentUpdateIdHeader = request.header(EXPO_CURRENT_UPDATE_ID_HEADER)
      ?: throw IOException("Cannot apply patch without $EXPO_CURRENT_UPDATE_ID_HEADER header")
    ...
    if (expectedBaseHash != null && actualBaseHash != null && expectedBaseHash != actualBaseHash) {
      ...
      throw IOException("Asset hash mismatch for update $currentUpdateIdHeader; expected=$expectedBaseHash actual=$actualBaseHash")
    }
